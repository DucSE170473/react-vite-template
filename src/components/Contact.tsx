            ...
            <div className="max-w-7xl mx-auto px-4 md:px-6">
                ...
                <div className="relative p-6 md:p-12 rounded-[2rem] md:rounded-[3rem] overflow-hidden group mb-12 shadow-xl">
                    ...
                    <div className="relative z-10 flex flex-col lg:flex-row lg:items-end justify-between gap-12">
                        ...
                        {/* Vế phải: Khu vực 2 mã QR */}
                        <div className="flex flex-row gap-4 md:gap-6 bg-white/5 backdrop-blur-md p-4 md:p-6 rounded-[2.5rem] border border-white/10 shadow-2xl">
                            {/* QR Công ty */}
                            <div className="flex flex-col items-center gap-3 group">
                                <div className="relative p-2 bg-white rounded-2xl overflow-hidden shadow-lg transition-transform duration-500 group-hover:scale-105 group-hover:-rotate-2">
                                    <img
                                        src="/QR-TTB-Corp.png"
                                        alt="QR Zalo Công ty"
                                ...
                                <div className="text-center">
                                    <p className="text-[8px] md:text-[10px] font-black text-white/40 uppercase tracking-[0.2em]">Zalo OA</p>
                                    <p className="text-[10px] md:text-xs font-bold text-white uppercase mt-0.5">TTB CORP</p>
                                </div>
                            </div>
                            ...
                            {/* QR Sếp */}
                            <div className="flex flex-col items-center gap-3 group">
                                <div className="relative p-2 bg-white rounded-2xl overflow-hidden shadow-lg transition-transform duration-500 group-hover:scale-105 group-hover:rotate-2 border-2 border-[var(--brand-accent)]/30">
                                    <img
                                        src="/tuan.jpg"
                                        alt="QR Zalo "
                                        className="w-20 h-20 md:w-28 md:h-28 object-contain"
                                    />
                                    <div className="absolute inset-0 bg-[var(--brand-accent)]/5 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                                </div>
                                <div className="text-center">
                                    <p className="text-[8px] md:text-[10px] font-black text-[var(--brand-accent)] uppercase tracking-[0.2em]">Hotline 24/7</p>
                                    <p className="text-[10px] md:text-xs font-bold text-white uppercase mt-0.5 text-nowrap">Mr. Tuấn</p>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            {[
                                {
                                    ...
                                    content: "166/12 Huỳnh Văn Nghệ, P. Tân Sơn, TP. HCM"
                                },
                                {
                                    icon: <Phone size={24} />,
                                    label: "Hotline hỗ trợ",
                                    // Chuyển thành mảng để chứa đầy đủ các số bạn yêu cầu
                                    content: [
                                        "0792.51.51.51",
                                        "0946.234.114",
                                        "028.62.996.669",
                                ...
                                },
                                {
                                    icon: <Mail size={24} />,
                                    label: "Email công ty",
                                    content: "thanhcongtechco@gmail.com"
                                },
                                {
                                    icon: <Clock size={24} />,
                                    label: "Giờ làm việc",
                                    content: "Thứ 2 - Thứ 7: 08:00 - 17:30"
                                }
                            ].map((item, idx) => (
                                <div key={idx} className="flex gap-4 p-6 bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-md hover:border-[var(--brand-primary)]/20 transition-all group">
                                    ...
                                    <div className="overflow-hidden w-full">
                                        ...
                                        {/* Logic hiển thị linh hoạt: Nếu là mảng thì map ra, nếu là string thì hiện bình thường */}
                                        <div className={`mt-1 leading-snug flex flex-col gap-1.5 ${item.highlight ? 'text-[var(--brand-primary)]' : 'text-slate-700'}`}>
                                            {Array.isArray(item.content) ? (
                                                item.content.map((phone, pIdx) => (
                                                    <a
                                                        key={pIdx}
                                                        href={`tel:${phone.replace(/\./g, '')}`}
                                                        className="text-sm md:text-base font-black hover:underline underline-offset-4 transition-all flex items-center gap-2"
                                                    >
                                                        {phone}
                                                        <span className="text-[8px] opacity-0 group-hover:opacity-100 bg-[var(--brand-primary)] text-white px-1.5 py-0.5 rounded uppercase tracking-tighter">Gọi</span>
                                                    </a>
                                                ))
                                            ) : (
